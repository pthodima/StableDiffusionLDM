...
class DDPM(nn.Module):
    ...
    def p_sample(self, x, label, t, t_index):
        ...
        # Calculate mean (mu) for the posterior q(x_{t-1} | x_t, x_0)
        mu = sqrt_recip_alphas_t * (x - betas_t / sqrt_one_minus_alphas_cumprod_t * pred_noise)

        # Add noise if not the last step
        if t_index > 0:
            noise = torch.randn_like(x)
            x_prev = mu + torch.sqrt(self.posterior_variance[t_index]) * noise
        else:
            x_prev = mu
    ...
    def p_sample_loop(self, labels):
        ...
        for t_index in reversed(range(self.timesteps)):
            t = torch.full((len(labels),), t_index, device=device, dtype=torch.long)
